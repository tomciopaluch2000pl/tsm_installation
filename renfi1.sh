...
#
# Notes:
#   - Default is DRY-RUN (prints what would be executed). Use --run to apply.
...
# CSV-safe splitter (respects quotes). Nie dotykamy liczby kolumn; trailing empties zachowamy przy 'read' dzięki sentinelowi.
split_csv_line() {
...
tail -n +2 "$CSV_FILE" | while IFS= read -r line; do
  ...
  # TRICK: dodajemy na końcu separator i SENTINEL, żeby bash zachował puste trailing fields.
  # Potem usuwamy ostatni element (=SENTINEL).
  IFS="|" read -r -a arr <<< "${parsed}|__SENTINEL__"
  unset 'arr[${#arr[@]}-1]'

  # GUARD: jeśli mimo wszystko brakuje indeksów/kolumn → zgłoś w raporcie
