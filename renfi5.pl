...
#   - Uses `df -Pk` (POSIX format) + `uname -s` (Linux vs AIX) to read free KB.
#   - Requirement: free_bytes >= input_csv_size * 2.2
#     (~report ≤ input + rollback ≤ input + 20% safety margin).
#
# dsmadmc flags:
...
# Pretty-print bytes in KB/MB/GB/TB with 1 decimal (except for pure bytes)
sub human_bytes {
    my ($b) = @_; $b ||= 0;
    my @u = ('B','KB','MB','GB','TB');
    my $i = 0;
    while ($b >= 1024 && $i < $#u) { $b /= 1024; $i++ }
    return $i == 0 ? sprintf('%d %s', $b, $u[$i])
                   : sprintf('%.1f %s', $b, $u[$i]);
}
...
sub check_disk_space_for_outputs {
    ...
    if ($avail_bytes < $need_bytes) {
        die sprintf("ERROR: Not enough free space in output filesystem (%s). Need >= %s, available %s.\n",
                    $out_dir, human_bytes($need_bytes), human_bytes($avail_bytes));
    }
    printf STDERR "INFO : Disk space OK in '%s' (need ~%s, available %s).\n",
                  $out_dir, human_bytes($need_bytes), human_bytes($avail_bytes);
}
