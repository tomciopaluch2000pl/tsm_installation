...
#
# USAGE:
#   perl add_instance_2_tsm_client.pl --servername <NAME> \
#        --tcpserveraddress <HOSTNAME> \
#        --tcpport <PORT> [--dry-run] [--force]
#
# OPTIONS:
#   --servername        Instance/server name (e.g., ODLAHU01).
#   --tcpserveraddress  TCP server address (FQDN or IP).
#   --tcpport           TCP port number.
#   --dry-run           Simulate actions; no changes applied.
#   --force             Overwrite instance files if they already exist.
#   --help              Print usage instructions.
#
# NOTES:
#   - Must be executed as root (unless using --dry-run).
...
# -------- CLI --------
my ($servername, $tcpserveraddress, $tcpport, $dry_run, $force, $help);
GetOptions(
