...
# CSV-safe splitter (respects quotes). Trailing empty fields are preserved using a sentinel trick at read-time.
split_csv_line() {
...
# Trim helper:
# - remove trailing CR (Windows), leading/trailing spaces, and outer quotes.
...
tail -n +2 "$CSV_FILE" | while IFS= read -r line; do
  ...
  # Strip a possible trailing CR from the raw line (CRLF case).
  line="${line%$'\r'}"

  parsed="$(split_csv_line "$line")"

  # SENTINEL TRICK:
  # Add a final separator + sentinel so bash preserves trailing empty fields,
  # then drop the sentinel element.
  IFS="|" read -r -a arr <<< "${parsed}|__SENTINEL__"
  unset 'arr[${#arr[@]}-1]'

  # GUARD: ensure we have at least as many fields as header advertised.
  if (( ${#arr[@]} < expected_cols || idx_node>=${#arr[@]} || idx_src_inst>=${#arr[@]} || idx_src_agid>=${#arr[@]} || idx_dst_inst>=${#arr[@]} || idx_dst_agid>=${#arr[@]} )); then
