...
# - CRLF is fine; we open with UTF-8 and trim CR in clean()
...
while (my $row = $csv->getline_hr($fh)) {
    # 1) Skip if no node (CMOD used only CACHE)
    ...
    # 7) Rollback line only if we actually changed something
