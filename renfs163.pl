...
# Team:       TahD
# Date:       2025-08-29
# Version:    1.6.3 (batch-only, readable; dsmadmc forced -noc)
#
# Change in 1.6.3:
#   - All dsmadmc calls now include -noc (non-console mode) so no interactive
#     confirmations are required. We keep -comma for stable parsing; we do NOT
#     use -dataonly=yes per your request.
#
...
# Build dsmadmc command line (ALWAYS with -noc)
sub build_dsmadmc_cmd {
    my ($tsm_cmd) = @_;
    my @cmd = ("dsmadmc", "-comma", "-noconfirm", "-noc");  # <- -noc is key here

    # Optional targeting / credentials
    push @cmd, "-servername=$SERVERNAME" if $SERVERNAME;
    push @cmd, "-id=$USER"               if $USER;
    push @cmd, "-password=$PASS"         if defined $PASS && $PASS ne "";

    # TSM command/macro to run
...
# Query filespaces for one node (single call per node)
...
sub batch_execute {
    ...
    # -noc is already part of build_dsmadmc_cmd()
...
Notes:
  - All dsmadmc calls run with '-noc' to avoid interactive prompts.
  - Default is DRY-RUN (no changes) until --run is used.
  - dsmadmc also uses '-comma' for stable parsing (no -dataonly used).
  - Required CSV headers: OD_INSTAME_SRC, AGID_NAME_SRC, OD_INSTAME_DST, AGID_NAME_DST, OD_TSM_LOGON_NAME

Outputs:
  - Report CSV   : fs_rename_report_<timestamp>.csv
  - Rollback CSV : fs_rollback_csv_<timestamp>.csv (inverse mappings)
  - Macro file   : fs_batch_<timestamp>.mac (deleted unless --keep-batch)

Examples:
  ./cmod_fs_rename.pl -c mapping.csv --delim ';'                            # dry-run
  ./cmod_fs_rename.pl -c mapping.csv --servername DRARD11B -u ondem -p XXX --run --verify
  ./cmod_fs_rename.pl -c mapping.csv --run --undo --verify --keep-batch
USAGE
}