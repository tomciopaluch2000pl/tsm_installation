...
# Team:       TahD
# Date:       2025-08-27
# Version:    1.6.1 (batch-only, undo-aware reporting)
#
# High-level flow:
...
#      - Forward mode:
#          * Skip when old_fs missing or new_fs already exists (safe).
#      - Undo mode (improved reporting):
#          * If new_fs exists AND old_fs missing  -> OK (already rolled back).
#          * If new_fs exists AND old_fs exists  -> WARN (both present) – manual check.
#          * Otherwise, behave like forward pre-checks.
...
#        --verify-late  : same idea, performed as a final pass.
...
while (my $row = $csv_in->getline_hr($fh)) {
    ...
    if ($UNDO) {
        ...
        my $has_new = $FS_CACHE{$node}{$new_fs} ? 1 : 0;  # new = /SRC/AGID (target of undo)

        if ($has_new && !$has_old) {
            # Already rolled back earlier
            report_add_row(node=>$node, old_fs=>$old_fs, new_fs=>$new_fs,
                           action=>"rename", status=>"OK", message=>"already rolled back");
            next;
        }
        if ($has_new && $has_old) {
            # Both present: do not risk renaming; manual decision needed
            report_add_row(node=>$node, old_fs=>$old_fs, new_fs=>$new_fs,
                           action=>"rename", status=>"WARN", message=>"both old and new present; manual check");
            next;
        }
        # else fall through to standard checks (has_old==1, has_new==0 expected)
    }

    # ---------------- Standard pre-checks (forward or remaining undo cases) ----------------
    unless ($FS_CACHE{$node}{$old_fs}) {
        report_add_row(node=>$node, old_fs=>$old_fs, new_fs=>$new_fs, action=>"rename",
    ...
    # Mark node for possible verification
...
Direction:
  ...
  --undo : old_fs=/OD_INSTAME_DST/AGID_NAME_DST  -> new_fs=/OD_INSTAME_SRC/AGID_NAME_SRC
           (If new_fs exists and old_fs is missing: OK 'already rolled back';
            If both exist: WARN 'both old and new present' and skip queuing.)
...
Verification:
  ...
  --verify-late  : same idea, but performed as a final pass.
