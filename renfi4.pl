...
#   - Before writing outputs, we check free space on the output filesystem (cwd).
#   - Implementation uses `df -Pk` (POSIX format) and `uname -s` to select
#     the right column for "Available/Free":
#       Linux: column 4 = Available (in KB), AIX: column 3 = Free (in KB).
#   - Requirement: free_bytes >= input_csv_size * 2.2
#       (≈ report ≤ input, rollback ≤ input, plus ~20% safety margin).
#
# dsmadmc flags:
...
while (my $row = $csv->getline_hr($fh)) {
    ...
    # Rollback CSV row: inverse mapping of what we just applied
...
sub check_disk_space_for_outputs {
    ...
    my $need_bytes = int($input_size * 2.2);  # report (~input) + rollback (~input) + 20% margin
    my $out_dir = ".";                    # outputs written to current working directory

    my $os = `uname -s`; chomp($os);
    my $df  = `df -Pk $out_dir 2>/dev/null`;
    my @lines = split /\n/, $df;
    if (@lines < 2) {
        print STDERR "WARN : Could not parse 'df -Pk' output; skipping free-space check.\n";
        return;
    }
    my @fields = split /\s+/, $lines[-1];

    # Linux POSIX: Filesystem 1024-blocks Used Available Capacity Mounted on  -> Available = idx 3
    # AIX 7.2   : Filesystem 1024-blocks Free Used %Used Iused %Iused Mounted on -> Free = idx 2
    my $avail_kb;
    if ($os eq 'AIX') { $avail_kb = $fields[2]; }
    else              { $avail_kb = $fields[3]; }

    $avail_kb = 0 unless defined $avail_kb && $avail_kb =~ /^\d+$/;
    my $avail_bytes = $avail_kb * 1024;

    if ($avail_bytes < $need_bytes) {
        die sprintf("ERROR: Not enough free space in output filesystem (%s). Need >= %d bytes, available %d bytes.\n",
                    $out_dir, $need_bytes, $avail_bytes);
    }
    printf STDERR "INFO : Disk space OK in '%s' (need ~%d B, available %d B).\n",
                  $out_dir, $need_bytes, $avail_bytes);
}
